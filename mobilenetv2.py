...
class MobileNetV2(nn.Module):
    def __init__(self, n_class=1000, input_size=224, width_mult=1.,use_attention=False):
        ...
        if self.use_attention:
            self.attention = CBAM(channels=self.last_channel)
...
def mobilenetv2(pretrained=False, use_attention=False, **kwargs):
    
    model = MobileNetV2(n_class=1000, use_attention=use_attention, **kwargs)
    if pretrained:
